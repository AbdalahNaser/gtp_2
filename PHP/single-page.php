...
<?php
if(isset($_POST['quantity']) && isset($_POST['price'])&& isset($_POST['part_name']))
{
  ...
  if ($conn->connect_error) {
    ...
    $Insert = "INSERT INTO part(quantity,price,part_name) values(?,?,?)";
    $stmt = $conn->prepare($Insert);
    $stmt->bind_param("iis",$quantity,$price,$part_name);
    if($stmt->execute()) {
      echo"
